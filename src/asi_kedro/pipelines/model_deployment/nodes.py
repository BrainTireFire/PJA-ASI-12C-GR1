...
def evaluate_model(challenger: TabularPredictor | LinearRegression, champion: TabularPredictor | LinearRegression, test_data: pd.DataFrame, params_auto_ml) -> Tuple[TabularPredictor | LinearRegression]:
    try:
        winner = {}
        r2_champion = None
        r2_challenger = None
        ...
        if isinstance(champion, TabularPredictor):
            ...
            r2_champion = -champion.evaluate(test_data)['root_mean_squared_error']
        
        if isinstance(champion, LinearRegression):
            print('Champion is a LinearRegression')
            x_test = test_data.drop(columns=[params_auto_ml.get('label_column', 'math_score')], inplace=False)
            y_test = test_data[params_auto_ml.get('label_column', 'math_score')]
        ...
        if isinstance(challenger, TabularPredictor):
            ...
            r2_challenger = -challenger.evaluate(test_data)['root_mean_squared_error']
